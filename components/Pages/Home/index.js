...
class App extends Component {
  ...
  _setCurrentMovieDetails = (movieId) => {
    const { data } = this.props.movies;

    const currentMovieDetails = data.find((movie) => movie.imdbID === movieId);

    this.setState({
      selectedMovieDetails: { ...currentMovieDetails },
    });

    this.props.history.push({
      pathname: "/CurrentMovieDetails",
      aboutProps: currentMovieDetails,
    });
    console.log(currentMovieDetails, "currentMovieDetails");
  };
          ...
          <div className="row">
            {loading ? (
              ...
              data.map((item) => {
                ...
                return <Movies imdbID={item.imdbID} image={item.Poster} name={item.Title} year={item.Year} type={item.Type} _setCurrentMovieDetails={this._setCurrentMovieDetails} />;
              })
            )}
